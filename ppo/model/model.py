...
class Model(nn.Module):
    def __init__(self):
        ...
        self.lstm_hidden_dim = Config.LSTM_REAL_SIZE
        ...
        self.hero_id_feature_dim = int(np.sum(DimConfig.DIM_OF_HERO_ID_INFO))
        self.monster_feature_dim = int(np.sum(DimConfig.DIM_OF_MONSTER_INFO))
        self.cake_feature_dim = int(np.sum(DimConfig.DIM_OF_CAKE_INFO))
        self.bullet_feature_dim = int(np.sum(DimConfig.DIM_OF_BULLET_INFO))
        self.buff_feature_dim = int(np.sum(DimConfig.DIM_OF_BUFF_INFO))
        self.command_feature_dim = int(np.sum(DimConfig.DIM_OF_COMMAND_INFO))
        self.target_feature_dim = int(np.sum(DimConfig.DIM_OF_TARGET_INFO))
        ...
        """ hero_share module"""
        fc_hero_dim_list = [self.single_hero_feature_dim + self.hero_id_feature_dim, 512, 256, 128]
        ...
        """ monster module"""
        fc_monster_dim_list = [self.monster_feature_dim, 32, 16]
        self.monster_mlp = MLP(fc_monster_dim_list, "monster_mlp")

        """ bullet module"""
        fc_bullet_dim_list = [self.bullet_feature_dim + self.hero_id_feature_dim, 64, 32]
        self.bullet_mlp = MLP(fc_bullet_dim_list, "bullet_mlp")

        """ buff module"""
        fc_buff_dim_list = [self.buff_feature_dim + self.hero_id_feature_dim, 256, 64, 32]
        self.buff_mlp = MLP(fc_buff_dim_list, "buff_mlp")

        """ command module"""
        fc_command_dim_list = [self.command_feature_dim + self.hero_id_feature_dim, 128, 32]
        self.command_mlp = MLP(fc_command_dim_list, "command_mlp")

        """ target info module"""
        fc_target_info_dim_list = [self.target_feature_dim + self.hero_id_feature_dim, 128, 32]
        self.target_info_mlp = MLP(fc_target_info_dim_list, "target_info_mlp")       
        ...
        concat_dim = (
            ...
            + 8 * fc_soldier_dim_list[-1]
            + 1 * fc_monster_dim_list[-1]
            + 1 * fc_bullet_dim_list[-1]
            + 1 * fc_buff_dim_list[-1]
            + 1 * fc_command_dim_list[-1]
            + 1 * fc_target_info_dim_list[-1]
            + 2 * fc_organ_dim_list[-1]
            + self.cake_feature_dim
            + self.hero_id_feature_dim
        ...
        self.lstm = torch.nn.LSTM(
            ...
            hidden_size=self.lstm_real_size,
            num_layers=1,
            bias=True,
            batch_first=True,
            dropout=0,
        ...
        self.label_mlp = ModuleDict(
            {
                "hero_label{0}_mlp".format(label_index): MLP(
                    [self.lstm_unit_size + extra_dim[label_index], 128, self.label_size_list[label_index]],
        ...
        """output value"""
        # self.multihead_value_mlp = MLP([self.lstm_unit_size, 128, 64, 5], "hero_multihead_value_mlp")
        self.value_mlp = MLP([self.lstm_unit_size, 128, 64, 1], "hero_value_mlp")

        self.target_embed_mlp = make_fc_layer(32, self.target_embed_dim, use_bias=False)

    def forward(self, data_list, inference=True):
        feature_vec = data_list[0] 
        lstm_hidden_init = data_list[1]
        lstm_cell_init = data_list[2]
        ...
        feature_vec_split_list = feature_vec.split(
            [
                ...
                self.global_feature_dim,
                self.hero_id_feature_dim,
                self.monster_feature_dim,
                self.cake_feature_dim,
                self.bullet_feature_dim,
                self.buff_feature_dim,
                self.command_feature_dim,
        ...
        hero_vec_list = feature_vec_split_list[0].split(
            [
                int(sum(DimConfig.DIM_OF_HERO_FRD)),
                int(sum(DimConfig.DIM_OF_HERO_EMY)),
                int(sum(DimConfig.DIM_OF_HERO_MAIN)),
            ],
            dim=1,
        )
        soldier_vec_list = feature_vec_split_list[1].split(
            [
                int(sum(DimConfig.DIM_OF_SOLDIER_1_10)),
                int(sum(DimConfig.DIM_OF_SOLDIER_11_20)),
            ],
            dim=1,
        )
        organ_vec_list = feature_vec_split_list[2].split(
            [
                int(sum(DimConfig.DIM_OF_ORGAN_1_2)),
                int(sum(DimConfig.DIM_OF_ORGAN_3_4)),
            ],
            dim=1,
        )
        global_info_list = feature_vec_split_list[3]
        hero_id_info_list = feature_vec_split_list[4]
        monster_info_list = feature_vec_split_list[5]
        cake_info_list = feature_vec_split_list[6]
        bullet_info_list = feature_vec_split_list[7]
        buff_info_list = feature_vec_split_list[8]
        command_info_list = feature_vec_split_list[9]
        target_info_list = feature_vec_split_list[10]
        ...
        _monster = monster_info_list
        _bullet_input = torch.cat([bullet_info_list, hero_id_info_list], dim=1)
        _buff_input = torch.cat([buff_info_list, hero_id_info_list], dim=1)
        _command_info_input = torch.cat([command_info_list, hero_id_info_list], dim=1)
        _target_info_input = torch.cat([target_info_list, hero_id_info_list], dim=1)
        ...
        # hero_main
        # 英雄主向量
        main_hero = self.hero_main_mlp(_hero_main[0])
        hero_main_result = main_hero

        hero_emy_result_list = []
        for index in range(len(_hero_emy)):
            hero_emy_mlp_out = self.hero_mlp(torch.cat([_hero_emy[index], hero_id_info_list], dim=1))
        ...
        for index in range(len(_hero_frd)):
            hero_frd_mlp_out = self.hero_mlp(torch.cat([_hero_frd[index], hero_id_info_list], dim=1))
        ...
        # monster
        # 野怪处理
        monster_mlp_out = self.monster_mlp(_monster)

        # bullet
        # 子弹处理
        bullet_mlp_out = self.bullet_mlp(_bullet_input)

        # buff
        # 增益处理
        buff_mlp_out = self.buff_mlp(_buff_input)

        # command
        # 技能释放处理
        command_mlp_out = self.command_mlp(_command_info_input)

        # target info
        # 目标处理
        target_info_mlp_out = self.target_info_mlp(_target_info_input)
        ...
        reshape_pool_frd_soldier = soldier_frd_concat_result.reshape(-1, 128)
        ...
        reshape_pool_emy_soldier = soldier_emy_concat_result.reshape(-1, 128)
        ...
        parallel_outputs = self.parallel_mlp(reshape_fc_public_result)
        ...
        # concat
        reshape1 = lstm_outputs.reshape(-1, self.lstm_real_size)
        reshape2 = parallel_outputs.reshape(-1, self.lstm_unit_size - self.lstm_real_size)
        concat_outputs = torch.cat((reshape1, reshape2), dim=1)
        reshape_lstm_outputs_result = concat_outputs.reshape(-1, self.lstm_unit_size)
        ...
        for label_index, label_dim in enumerate(self.label_size_list[:-1]):
            if label_dim == 16:
                label_mlp_out = self.label_mlp["hero_label{0}_mlp".format(label_index)](torch.cat([reshape_lstm_outputs_result, extra_input], dim=1))
            else:
                label_mlp_out = self.label_mlp["hero_label{0}_mlp".format(label_index)](reshape_lstm_outputs_result)
                extra_input = label_mlp_out
    ...
    def compute_loss(self, data_list, rst_list):
        ...
        # 值网络的损失
        # multi-head value
        # fc2_value_result_squeezed = value_result
        # new_advantage = reward - fc2_value_result_squeezed
        # cost_mean = torch.mean(torch.square(new_advantage), dim=0)
        # self.value_cost = 0.5 * torch.sum(cost_mean)
        fc2_value_result_squeezed = value_result.squeeze(dim=1)   
        new_advantage = reward - fc2_value_result_squeezed
        batch_value_loss = torch.square(new_advantage)
        self.value_cost = 0.5 * torch.mean(batch_value_loss, dim=0)        
        ...
        for task_index in range(len(self.is_reinforce_task_list)):
            if self.is_reinforce_task_list[task_index]:
                ...
                c = 3
                # adv_sum = torch.sum(advantage, dim=1)
                adv_sum = advantage

                surr1 = clip_ratio * adv_sum
                surr2 = ratio.clamp(1.0 - self.clip_param, 1.0 + self.clip_param) * adv_sum
                surr3 = torch.minimum(c*adv_sum, -c*adv_sum)
                temp_policy_loss = -torch.sum(
                    torch.maximum(torch.minimum(surr1, surr2), surr3) * (weight_list[task_index].float()) * 1
...
class MLP(nn.Module):
    ...
    def __init__(
        ...
        for i in range(len(fc_feat_dim_list) - 1):
            ...
            # 添加 tanh 激活函数
            if i + 1 < len(fc_feat_dim_list) - 1 or non_linearity_last:
                self.fc_layers.add_module("{0}_tanh{1}".format(name, i + 1), nn.Tanh())
